...
if __name__ == '__main__':
    voc_folder = r"F:\datasets\SafetyHelmet\temp\Annotations" #储存voc格式的标注文件的文件夹，相对路径
    yolo_folder = r"F:\datasets\SafetyHelmet\temp\labels" #转换后的yolo格式标注文件的储存文件夹，相对路径
    class_id = ["hat","face"] #储存数据集中目标种类名称的列表，接下来的转换函数中会将该列表中种类名称对应的列表索引号作为写入yolo标注文件中该类目标的种类序号
